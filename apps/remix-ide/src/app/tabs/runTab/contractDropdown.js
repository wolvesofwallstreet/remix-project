...
class ContractDropdownUI {
      ...
      (success, data, source, compiler, compilerFullName, file) => {
        ...
        if (success) {
          this.dropdownLogic
            ...
            .forEach(contract => {
              this.selectContractNames.appendChild(
                yo`<option value="${contract.name}" compiler="${compilerFullName}">${contract.name} - ${contract.file}</option>`
              );
            });
        }
    ...
    this.blockchain.event.register('contextChanged', () => {
      this.blockchain.updateNetwork((err, { name } = {}) => {
        ...
        const savedConfig = window.localStorage.getItem(
          `ipfs/${this.exEnvironment}/${this.networkName}`
        );
  ...
  enableContractNames(enable) {
    if (enable) {
      ...
      this.selectContractNames.setAttribute(
        'title',
  ...
  render() {
    ...
    this.atAddress = yo`<button class="${
      css.atAddress
    } btn btn-sm btn-info" id="runAndDeployAtAdressButton" onclick=${this.loadFromAddress.bind(
      this
    )}>At Address</button>`;
    ...
    if (!this.atAddressButtonInput.value) {
      ...
      if (
        (this.selectContractNames &&
          !this.selectContractNames.getAttribute('disabled') &&
          this.loadType === 'sol') ||
        this.loadType === 'abi'
      ) {
  ...
  async createInstance(selectedContract, args) {
    ...
    var finalCb = (error, contractObject, address) => {
      ...
      if (this.ipfsCheckedState) {
        publishToStorage(
          'ipfs',
          this.runView.fileProvider,
          this.runView.fileManager,
          selectedContract
        );
      }
    };
    ...
    try {
      contractMetadata = await this.runView.call(
        ...
        selectedContract.contract.file
      );
    } catch (error) {
      return statusCb(
      ...
      </div>`,
        {
          ...
          fn: () => {
            this.deployContract(
              ...
              contractMetadata,
              compilerContracts,
              { continueCb, promptCb, statusCb, finalCb },
              confirmationCb
            );
          }
        },
  ...
  deployContract(
    ...
    if (
      ...
      return this.blockchain.deployContractAndLibraries(
        selectedContract,
        args,
        contractMetadata,
        compilerContracts,
    ...
    this.blockchain.deployContractWithLibrary(
      selectedContract,
      args,
      contractMetadata,
      compilerContracts,
  ...
  getConfirmationCb(modalDialog, confirmDialog) {
    ...
    const confirmationCb = (
      ...
      const content = confirmDialog(
        ...
        gasEstimation,
        null,
        this.blockchain.determineGasFees(tx),
        this.blockchain.determineGasPrice.bind(this.blockchain)
      );
      ...
      modalDialog(
        ...
        content,
        {
          label: 'Confirm',
          fn: () => {
            this.blockchain.config.setUnpersistedProperty(
            ...
            if (!content.gasPriceStatus) {
              ...
              var gasPrice = this.blockchain.toWei(
