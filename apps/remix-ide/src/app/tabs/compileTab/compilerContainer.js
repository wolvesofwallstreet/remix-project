...
class CompilerContainer {
  ...
  set currentFile(name = '') {
    ...
    this._disableCompileBtn(
      !name ||
        (name &&
          !this.isSolFileSelected(name) &&
          !this.isJsonFileSelected(name))
    );
  ...
  isSolFileSelected(currentFile = '') {
    ...
    return (
      currentFile.toLowerCase().endsWith('.sol') ||
      currentFile.toLowerCase().endsWith('.yul')
    );
  }

  isJsonFileSelected(currentFile = '') {
    if (!currentFile) currentFile = this.config.get('currentFile');
    if (!currentFile) return false;
    return currentFile.toLowerCase().endsWith('.json');
  }
  ...
  compilationButton(name = '') {
    ...
    const jsonSelected = this.isJsonFileSelected();
    const disabled =
      name && (this.isSolFileSelected() || jsonSelected) ? '' : 'disabled';
    const title = jsonSelected ? 'Analyze' : 'Compile';

    return yo`
      <button id="compileBtn" data-id="compilerContainerCompileBtn" class="btn btn-primary btn-block ${disabled} mt-3" title="${title}" onclick="${this.compile.bind(
      this
    )}">
        <span>${this._view.compileIcon} ${title} ${displayed}</span>
      </button>
    `;
  }
  ...
  _disableCompileBtn(shouldDisable) {
    ...
    } else if (this.isSolFileSelected() || this.isJsonFileSelected()) {
  ...
  _setCompilerVersionFromPragma(filename) {
    ...
    this.compileTabLogic.fileManager.readFile(filename).then(data => {
      ...
      if (pragmaArr && pragmaArr.length === 1) {
        ...
        const releasedVersions = this.data.allversions
          .filter(obj => !obj.prerelease)
          .map(obj => obj.version);
        const allVersions = this.data.allversions.map(obj =>
          this._retrieveVersion(obj.version)
        );
        const currentCompilerName = this._retrieveVersion(
        ...
        if (isOfficial && !isInRange && !isNewestNightly) {
          ...
          const compilerPath = this.data.allversions.filter(
  ...
  render() {
    ...
    this._view.autoCompile = yo`<input class="${
      ...
      this.updateAutoCompile()} data-id="compilerContainerAutoCompile" id="autoCompile" type="checkbox" title="Auto compile">`;
    this._view.hideWarningsBox = yo`<input class="${
      css.autocompile
    } custom-control-input" onchange=${() =>
      this.hideWarnings()} id="hideWarningsBox" type="checkbox" title="Hide warnings">`;
    ...
    this._view.optimize = yo`<input onchange=${() =>
      this.onchangeOptimize()} class="custom-control-input" id="optimize" type="checkbox">`;
          ...
          <header class="${css.compilerSection} border-bottom">
            <div class="mb-2">
              <label class="${
                ...
                Compiler
                <button class="far fa-plus-square border-0 p-0 mx-2 btn-sm" onclick="${e =>
                  this.promtCompiler(
                    e
  ...
  promtCompiler() {
    modalDialogCustom.prompt('Add a custom compiler', 'URL', '', url =>
  ...
  compile(event) {
    ...
    if (!this.isSolFileSelected() && !this.isJsonFileSelected()) return;

    this._setCompilerVersionFromPragma(currentFile);
    this.compileTabLogic.runCompiler(this.isJsonFileSelected());
  }
  ...
  */
  ...
  _updateVersionSelector(customUrl = '') {
    ...
    if (
      ...
      !this._shouldBeAdded(this.data.selectedVersion)
    ) {
    ...
    if (customUrl !== '') {
      ...
      this._view.versionSelector.appendChild(
        yo`<option value="${customUrl}" selected>custom</option>`
      );
      url = customUrl;
      this.queryParams.update({ version: this.data.selectedVersion });
    ...
    if (
      this.data.selectedVersion !== 'builtin' &&
      canUseWorker(this.data.selectedVersion)
    ) {
  ...
  _updateLanguageSelector() {
    ...
    if (
      !semver.valid(this._retrieveVersion()) ||
      semver.lt(this._retrieveVersion(), 'v0.5.7+commit.6da8b019.js')
    ) {
  ...
  async fetchAllVersion(callback) {
    ...
    try {
      allVersions = JSON.parse(binRes.json)
        .builds.slice()
        .reverse();
      ...
      if (wasmRes.event.type !== 'error') {
        allVersionsWasm = JSON.parse(wasmRes.json)
          .builds.slice()
          .reverse();
      }
    } catch (e) {
